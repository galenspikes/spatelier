...
def test_video_download_command(mock_service_factory_class, cli_runner):
    ...
    result = cli_runner.invoke(video_app, [
        'download',
...
def test_audio_info_command(mock_get_audio_info, cli_runner):
    ...
    try:
        result = cli_runner.invoke(audio_app, [
            'info',
            str(test_file)
        ])
...
def test_utils_hash_command(cli_runner):
    ...
    try:
        result = cli_runner.invoke(utils_app, [
            'hash',
            str(test_file),
            '--algorithm', 'sha256'
        ])
        ...
        assert 'dffd6021bb2bd5b0af676290809ec3a53191dd81c7f70a4b28688a362182986f' in result.output
    finally:
        if test_file.exists():
            test_file.unlink()
...
def test_analytics_report_command(mock_reporter_class, cli_runner):
    ...
    result = cli_runner.invoke(analytics_app, [
        'report',
        '--days', '30'
    ])
