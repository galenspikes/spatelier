...
def test_metadata_extractor_parse_youtube_metadata(metadata_extractor):
    ...
    assert parsed['thumbnail_url'] == 'https://img.youtube.com/vi/test123/maxresdefault.jpg'
...
def test_metadata_extractor_extract_youtube_metadata(mock_ydl_class, metadata_extractor):
    ...
    result = metadata_extractor.extract_youtube_metadata('https://youtube.com/watch?v=test')
...
@patch('yt_dlp.YoutubeDL')
def test_metadata_extractor_extract_youtube_metadata_failure(mock_ydl_class, metadata_extractor):
    """Test YouTube metadata extraction failure."""
    # Mock failed yt-dlp output
    mock_ydl_instance = Mock()
    mock_ydl_instance.extract_info.side_effect = Exception('ERROR: [youtube] test: Video unavailable')
    mock_ydl_class.return_value.__enter__.return_value = mock_ydl_instance
...
def test_metadata_extractor_extract_file_metadata(mock_exists, mock_probe, metadata_extractor):
...
def test_metadata_manager_enrich_media_file(metadata_manager):
    ...
    with patch.object(metadata_manager.extractor, 'extract_file_metadata') as mock_file_meta, \
         patch.object(metadata_manager.extractor, 'extract_youtube_metadata') as mock_youtube_meta, \
         patch.object(metadata_manager.extractor, 'update_media_file_metadata') as mock_update:
        
        mock_file_meta.return_value = {'duration': 120.5, 'width': 1920}
        mock_youtube_meta.return_value = {'title': 'Test Video', 'uploader': 'Test Channel'}
...
def test_metadata_extractor_update_media_file_metadata(metadata_extractor):
    ...
    result = metadata_extractor.update_media_file_metadata(media_file, metadata, repository)
