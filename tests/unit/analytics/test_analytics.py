...
from database.models import MediaFile, ProcessingJob, AnalyticsEvent, MediaType, ProcessingStatus
...
def test_generate_usage_report(analytics_reporter):
    ...
    analytics_reporter.repos.analytics.get_events_by_type.side_effect = mock_get_events_by_type
...
def test_create_visualizations(mock_subplots, mock_close, mock_savefig, analytics_reporter):
...
def test_export_data_csv(mock_open, mock_to_csv, analytics_reporter):
    ...
    # Mock report generation
    analytics_reporter.generate_media_report = Mock(return_value={
        'recent_files': [{'id': 1, 'name': 'test.mp4', 'size': 100000}]
    })
    analytics_reporter.generate_processing_report = Mock(return_value={
