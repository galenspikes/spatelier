...
from core.worker import Worker, WorkerMode, create_download_processor, create_playlist_processor
...
class TestWorkerStuckJobDetection:
    ...
    def test_get_stuck_jobs_recent(self, worker):
        ...
        recent_job = Job(
            ...
            started_at=datetime.now() - timedelta(seconds=10)  # Recent (less than timeout)
        )
        ...
        with patch.object(worker.job_queue, 'get_jobs_by_status', return_value=[recent_job]):
    ...
    def test_get_stuck_jobs_old(self, worker):
        ...
        with patch.object(worker.job_queue, 'get_jobs_by_status', return_value=[old_job]):
            with patch.object(worker, '_is_process_running', return_value=False):
                with patch.object(worker, '_is_job_making_progress', return_value=False):
                    stuck_jobs = worker._get_stuck_jobs()
...
class TestWorkerRetryLogic:
    ...
    def test_get_retryable_failed_jobs(self, worker):
        ...
        with patch.object(worker.job_queue, 'get_jobs_by_status', return_value=[retryable_job, non_retryable_job]):
...
class TestWorkerStatistics:
    ...
    def test_get_stats(self, worker):
        ...
        with patch.object(worker.job_queue, 'get_queue_status', return_value={'pending': 5}):
