...
from core.config import Config, VideoConfig, AudioConfig, DatabaseConfig, TranscriptionConfig
from core.logger import get_logger, setup_logging
from core.base import BaseProcessor, BaseDownloader, BaseConverter, ProcessingResult
from utils.helpers import (
    get_file_hash, get_file_size, format_file_size, get_file_type,
    is_video_file, is_audio_file, find_files, safe_filename
)
...
def test_video_config_defaults():
    ...
    assert config.temp_dir.name == "spatelier"
...
def test_config_ensure_default_config():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        with patch.object(Config, 'get_default_config_path') as mock_path:
            ...
            assert Path(temp_dir) / "config.yaml" in mock_path.return_value.parent.glob("*.yaml")
