...
class PerformanceMonitor:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_operation_time": statistics.mean([op["duration"] for op in self.operations]) if self.operations else 0,
            "max_operation_time": max([op["duration"] for op in self.operations]) if self.operations else 0,
            "min_operation_time": min([op["duration"] for op in self.operations]) if self.operations else 0,
            "peak_memory_usage": max(self.memory_usage) if self.memory_usage else 0,
            "average_memory_usage": statistics.mean(self.memory_usage) if self.memory_usage else 0,
            "peak_cpu_usage": max(self.cpu_usage) if self.cpu_usage else 0,
            "average_cpu_usage": statistics.mean(self.cpu_usage) if self.cpu_usage else 0
        }
...
def benchmark_file_operation(operation: Callable, file_path: Path, *args, **kwargs) -> Dict[str, Any]:
...
def benchmark_database_operation(operation: Callable, *args, **kwargs) -> Dict[str, Any]:
...
def benchmark_file_system_operation(operation: Callable, *args, **kwargs) -> Dict[str, Any]:
