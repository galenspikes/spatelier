...
def assert_file_hash_matches(
    ...
    if actual_hash != expected_hash:
        msg = message or f"File hash mismatch: expected {expected_hash}, got {actual_hash}"
        raise AssertionError(msg)
...
def assert_file_size_within_range(
    ...
    if not (min_size <= actual_size <= max_size):
        msg = message or f"File size {actual_size} not in range [{min_size}, {max_size}]"
        raise AssertionError(msg)
...
def assert_srt_content_valid(srt_content: str, message: str = None) -> None:
    ...
    while i < len(lines):
        ...
        if i >= len(lines) or '-->' not in lines[i]:
            msg = message or f"Invalid SRT timestamp at line {i+1}: {lines[i] if i < len(lines) else 'EOF'}"
            raise AssertionError(msg)
        i += 1
...
def assert_database_record_not_exists(
    session,
    model_class,
    **filters
) -> None:
...
def assert_processing_job_status(
    ...
    if job.status != expected_status:
        msg = message or f"Job {job_id} status mismatch: expected {expected_status}, got {job.status}"
        raise AssertionError(msg)
...
def assert_media_file_metadata(
    ...
    for key, expected_value in expected_metadata.items():
        ...
        if actual_value != expected_value:
            msg = message or f"Media file {media_file_id} metadata mismatch for {key}: expected {expected_value}, got {actual_value}"
            raise AssertionError(msg)
...
def assert_file_permissions(
    ...
    if actual_permissions != expected_permissions:
        msg = message or f"File permissions mismatch: expected {oct(expected_permissions)}, got {oct(actual_permissions)}"
        raise AssertionError(msg)
...
def assert_directory_permissions(
    ...
    if actual_permissions != expected_permissions:
        msg = message or f"Directory permissions mismatch: expected {oct(expected_permissions)}, got {oct(actual_permissions)}"
        raise AssertionError(msg)
