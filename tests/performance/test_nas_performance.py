...
class TestNASPerformance:
    ...
    def test_nas_file_write_performance(self, nas_test_directory: Path, nas_file_scenarios: Dict, nas_available: bool):
        ...
        for scenario_name, scenario in nas_file_scenarios.items():
            ...
            results[scenario_name] = {
                ...
                "size": scenario["size"],
                "throughput_mbps": (scenario["size"] / (1024 * 1024)) / write_time if write_time > 0 else 0
            }
        ...
        for scenario, metrics in results.items():
            print(f"{scenario}: {metrics['write_time']:.3f}s, {metrics['throughput_mbps']:.2f} MB/s")
    
    def test_nas_file_read_performance(self, nas_test_directory: Path, nas_file_scenarios: Dict, nas_available: bool):
        ...
        for scenario_name, scenario in nas_file_scenarios.items():
            ...
            results[scenario_name] = {
                "read_time": read_time,
                "size": scenario["size"],
                "throughput_mbps": (scenario["size"] / (1024 * 1024)) / read_time if read_time > 0 else 0
            }
        ...
        # Log performance results
        for scenario, metrics in results.items():
            print(f"{scenario}: {metrics['read_time']:.3f}s, {metrics['throughput_mbps']:.2f} MB/s")
    
    def test_nas_directory_operations_performance(self, nas_test_directory: Path, nas_available: bool):
    ...
    def test_nas_concurrent_file_operations(self, nas_test_directory: Path, nas_available: bool):
        ...
        # Verify no errors
        assert total_errors == 0, f"Errors occurred: {[r['errors'] for r in results if r['errors']]}"
    
    def test_nas_large_file_handling(self, nas_test_directory: Path, nas_available: bool):
        ...
        for size_mb, metrics in results.items():
            print(f"  {size_mb}MB: write={metrics['write_time']:.3f}s ({metrics['write_throughput']:.2f} MB/s), "
                  f"read={metrics['read_time']:.3f}s ({metrics['read_throughput']:.2f} MB/s)")
    
    def test_nas_network_resilience(self, nas_test_directory: Path, nas_available: bool):
        ...
        if operation_times:
            ...
            print(f"  Success rate: {(num_operations - len(errors)) / num_operations * 100:.1f}%")
            ...
            if first_half and second_half:
                ...
                assert degradation < 50, f"Significant performance degradation: {degradation:.1f}%"
    ...
    def test_nas_memory_usage(self, nas_test_directory: Path, nas_available: bool):
        ...
        print(f"  Final increase: {memory_usage['final_increase'] / 1024 / 1024:.2f} MB")
    ...
    def test_nas_throughput_benchmark(self, nas_test_directory: Path, nas_available: bool):
        ...
        for size, metrics in results.items():
            size_kb = size / 1024
            print(f"  {size_kb:.0f}KB: write={metrics['write_throughput']/1024:.2f} KB/s, "
