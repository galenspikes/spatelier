...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_media_files_file_device'), 'media_files', ['file_device'], unique=False)
    
    # Add file_inode column
    op.add_column('media_files', sa.Column('file_inode', sa.Integer(), nullable=True))
    op.create_index(op.f('ix_media_files_file_inode'), 'media_files', ['file_inode'], unique=False)
    
    # Add file_identifier column (device:inode string)
    op.add_column('media_files', sa.Column('file_identifier', sa.String(length=50), nullable=True))
    op.create_index(op.f('ix_media_files_file_identifier'), 'media_files', ['file_identifier'], unique=True)
